                ...
                @foreach ($favorite_products as $favorite_product)
                    ...
                    <div class="col-md-2 d-flex align-items-center justify-content-end">
                        <a href="{{ route('favorites.destroy', $favorite_product->id) }}"
                            ...
                            onclick="event.preventDefault(); document.getElementById('favorites-destroy-form{{ $favorite_product->id }}').submit();">削除</a>
                        <form id="favorites-destroy-form{{ $favorite_product->id }}"
                            action="{{ route('favorites.destroy', $favorite_product->id) }}" method="POST" class="d-none">
                            @csrf
                            @method('DELETE')
                        </form>
                    </div>
                    <div class="col-md-3 d-flex align-items-center justify-content-end">
                        <form method="POST" action="{{ route('carts.store') }}" class="m-3 align-items-end">
                            ...
                            <button type="submit" class="btn samuraimart-favorite-add-cart">カートに入れいる</button>
                        </form>
                    </div>
                @endforeach
            </div>
